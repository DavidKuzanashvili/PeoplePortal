...
{
    public class Validator : AbstractValidator<CreatePersonRequest>
    {
        public Validator(IStringLocalizer<CreatePersonRequest> localizer)
        {
            ...
            RuleFor(x => x.PersonalNumber)
                .Length(11)
                .WithMessage(localizer["invalid.length11"])
                .Matches(@"[0-9]")
                .WithMessage(localizer["invalid.onlynumbers"]);

            RuleFor(x => x.DateOfBirth)
                .Must(x => x <= minAllowedDOB)
                .WithMessage(localizer["invalid.age"]);
